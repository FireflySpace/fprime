...
function(fpp_autocoder_variables FPP_IMPORTS)
    # Resolve the current binary directory.  This is necessary because fpp tools run in the JVM.
    ...
    string(REGEX REPLACE ";" ","  FPRIME_LOCATIONS_COMMA_SEP "${FPRIME_LOCATIONS}")
    # fpp imports also need to be a comma-separated list
    string(REGEX REPLACE ";" ","  FPP_IMPORTS_COMMA_SEP "${FPP_IMPORTS}")

    # If imports exist, then we need to prepend the -i flag for fpp
    set(IMPORT_FLAGS)
    if (FPP_IMPORTS_COMMA_SEP)
        set(IMPORT_FLAGS "-i" "${FPP_IMPORTS_COMMA_SEP}")
    endif()

    # Now set the variables for the FPP autocoder in parent scope
    set(FPP_OUTPUT_DIRECTORY "${CMAKE_CURRENT_BINARY_DIR_RESOLVED}" PARENT_SCOPE)
    set(FPP_LOCATIONS_COMMA_SEP "${FPRIME_LOCATIONS_COMMA_SEP}" PARENT_SCOPE)
    set(FPP_IMPORT_FLAGS "${IMPORT_FLAGS}" PARENT_SCOPE)
endfunction(fpp_autocoder_variables)
...
function(fpp_setup_autocode MODULE_NAME AC_INPUT_FILES)
    ...
    if (TARGET refresh_info_cache)
        append_list_property("${FILE_DEPENDENCIES}" TARGET refresh_info_cache PROPERTY FPP_SOURCES)
    endif()

    # Separate the source files into the various types generated by the fpp tools.
    set(GENERATED_CPP)
    set(GENERATED_DICT)

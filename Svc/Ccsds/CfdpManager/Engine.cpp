...
Status::T Engine::serializeAndSendPduOnChannel(
    Channel& chan,
    PduBase& pdu
) {
    Fw::Buffer buffer;
...
Status::T Engine::sendFinAckStateless(Channel& chan, TransactionSeq tsn,
                                      EntityId finSrcEid, EntityId finDstEid,
                                      ConditionCode cc)
{
    ...
    ack.initialize(
        ...
        FILE_DIRECTIVE_FIN,             // directive being acknowledged
        1,                              // directive subtype code (always 1)
        cc,                             // echo the FIN's condition code
        ACK_TXN_STATUS_UNRECOGNIZED     // we no longer recognize this transaction
    );
    ...
    if (status == Fw::FW_SERIALIZE_OK) {
        ...
        if (txn == NULL)
        {
                ...
                sourceEid == this->m_manager->getLocalEidParam())
            {
                ...
                {
                    this->sendFinAckStateless(*chan, transactionSeq, sourceEid, destEid,
                                              fin.getConditionCode());

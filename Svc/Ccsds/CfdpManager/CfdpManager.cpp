...
void CfdpManager ::parametersLoaded() {
    // Emit the initial value of every parameter once loading completes by
    // driving each parameter through parameterUpdated().
    parameterUpdated(PARAMID_LOCALEID);
    parameterUpdated(PARAMID_OUTGOINGFILECHUNKSIZE);
    parameterUpdated(PARAMID_RXCRCCALCBYTESPERCYCLE);
    parameterUpdated(PARAMID_POSTINACTIVITYSENDRETRIES);
    parameterUpdated(PARAMID_FILEINDEFAULTCHANNEL);
    parameterUpdated(PARAMID_FILEINDEFAULTDESTENTITYID);
    parameterUpdated(PARAMID_FILEINDEFAULTCLASS);
    parameterUpdated(PARAMID_FILEINDEFAULTKEEP);
    parameterUpdated(PARAMID_FILEINDEFAULTPRIORITY);
    parameterUpdated(PARAMID_CHANNELCONFIG);
}

void CfdpManager ::parameterUpdated(FwPrmIdType id) {
    // Re-emit only the parameter that changed so ground can verify the active
    // configuration.
    Fw::ParamValid valid;
    switch (id) {
